...
$nodeScript = Join-Path $PSScriptRoot "core-flow.mjs"
...
function Invoke-CorePhase {
    ...
    if (Test-Path -LiteralPath $logPath -PathType Leaf) {
        $previous = "core-$Phase-previous-{0}.log" -f (Get-Date -Format "yyyyMMdd-HHmmss")
    ...
    try {
        ...
        $botOutput = Join-Path $serverRoot "core-$Phase-bot.log"
        & $NodeExe $nodeScript $Phase $Port PSOwner PSMember $BaseX $BaseZ 180000 *>&1 |
            Tee-Object -FilePath $botOutput
        if ($LASTEXITCODE -ne 0) {
            throw "$Lane $Phase bot flow failed with exit $LASTEXITCODE"
        }
        $phaseSucceeded = $true
    } finally {
        ...
        ($stdout.Result + [Environment]::NewLine + $stderr.Result) |
            Set-Content -LiteralPath (Join-Path $serverRoot "core-$Phase-console.log") -Encoding utf8
    }

    if ($phaseSucceeded) {
        Copy-Item -LiteralPath $logPath -Destination (
            Join-Path $serverRoot "logs/core-$Phase-pass.log"
        ) -Force
    }
}

Invoke-CorePhase -Phase create
Invoke-CorePhase -Phase verify
...
foreach ($phase in @("create", "verify")) {
    $log = Get-Content -Raw -LiteralPath (Join-Path $serverRoot "logs/core-$phase-pass.log")
...
Write-Host "PASS: $Lane completed create, restart, persistence, remote removal, and physical break flows."
